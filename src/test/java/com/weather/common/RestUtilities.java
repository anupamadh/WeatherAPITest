...
public class RestUtilities {
  ...
  /*      AuthenticationScheme authScheme =
                RestAssured.oauth(Auth.PERSONAL_KEY, Auth.CONSUMER_SECRET, Auth.ACCESS_TOKEN,
                        Auth.ACCESS_SECRET);*/
        REQUEST_BUILDER = new RequestSpecBuilder();
        REQUEST_BUILDER.setBaseUri(Path.BASE_URI);
        REQUEST_BUILDER.setContentType(ContentType.JSON);
        REQUEST_BUILDER.addQueryParam("appid", Auth.PERSONAL_KEY);
...
//		response.then().log().all();
    ...
    public static RequestSpecification setBody(RequestSpecification rspec, Object object){
        return rspec.body(object);
    }
}
